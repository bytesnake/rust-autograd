...
impl<'tensor, 'graph: 'tensor, 'r: 'tensor, 'l: 'tensor, F: Float> crate::graph::Graph<F> {
    #[inline]
    pub(crate) fn scoped(
        &'graph self,
    ...
    ///
    /// Note: the current implementation works but is unoptimized to use.
    ///
    /// ```
    /// use autograd as ag;
    ...
    /// Returns the (symbolic) shape of the input tensor
    ///
    /// ```
    /// use autograd as ag;
    ///
    ...
    ///
    /// ag::with(|g| {
    ///    let a: ag::Tensor<f32> = g.zeros(&[2, 4]);
    ///    let b: ag::Tensor<f32> = g.zeros(&[2, 3]);
    ///    let c = g.matmul_t(a, b, true, false);
    ///    assert_eq!(c.eval(&[]).unwrap().shape(), &[4, 3]);
    /// });
    ///
    /// ```
    ...
    ///
    /// ag::with(|g| {
    ///    let a: ag::Tensor<f32> = g.zeros(&[2, 3, 4, 2]);
    ///    let b: ag::Tensor<f32> = g.zeros(&[2, 3, 2, 3]);
    ...
    /// Permutes dimensions.
    ///
    /// It's like TensorFlow or NumPy'graph.
    ///
    /// ```
    /// use autograd as ag;
    ///
    ...
    /// });
    /// ```
    pub fn transpose<A, AT>(&'graph self, x: A, perm: &AT) -> Tensor<'tensor, 'graph, F>
    where
        A: AsRef<Tensor<'tensor, 'graph, F>> + Copy,
        AT: AsTensor<'tensor, 'graph, F>,
    {
        ...
        Tensor::builder()
            .set_inputs(&[x.as_ref(), &perm.as_tensor(self)])
            .build(self, op)
    }
