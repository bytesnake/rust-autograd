...
impl ag::op::Op<f32> for MultiOutputOp {
    ...
    fn compute(&self, ctx: &mut ag::runtime::OpComputeContext<f32>) {
...
#[test]
fn test_hook() {
    let a: ag::Tensor<f32> = ag::ones(&[4, 2]).show();
    let b: ag::Tensor<f32> = ag::zeros(&[2, 3]).show_shape();
    let c = ag::matmul(a, b).print("aaa");
    ag::eval(&[c], &[]);
}
