...
type Receiver<'tensor, 'view, T: Float> = crossbeam_channel::Receiver<OpEvalResult<'tensor, 'view, T>>;
type Sender<'tensor, 'view, T: Float> = crossbeam_channel::Sender<OpEvalResult<'tensor, 'view, T>>;
...
struct SubGraph<'tensor, F: Float> {
// source nodes in this graph
sources: FxHashSet<&'tensor Tensor<F>>,
// tensor -> Node(=its immediate successor nodes etc)
node_map: FxHashMap<&'tensor Tensor<F>, Node<'tensor, F>>
}

impl<'a, F: Float> SubGraph<'a, F> {
#[inline]
fn get(&self, k: &'a Tensor<F>) -> &Node<'a, F> {
                                              self.node_map.get(k).as_ref().unwrap()
                                                                                    }
}

// ライフタイム 'view を持ってるものは、その本体である borrowed ストレージが死んだら死ぬ。
// しかし実際、'view を持つ Evaluator は borrowed ストレージが死んでもずっと生き続けているため、それはだめだと言われている.
// 解決策は2つ。
// 1. Sender, Receiver に、'view を持つのをやめさせる。
//  これは sender が compute 結果である ArrayView をメインスレッドに返すのをやめさせることが必要。だがそれは厳しい。。。
//
// 2. Sender は内部に view を保持しているわけではないのに 'view を要求してくるコト自体がおかしいため、それの根本解決をする
//
...
init_required: bool,
storage: EvaluatorStorage<'tensor, 'view, F>,
}
...
impl<'tensor, 'lock, F: Float> GuardState<'tensor, 'lock, F> {
fn invalidate(&mut self, is_mut: bool, node: &Tensor<F>, i: usize) {
unsafe {
if is_mut {
mem::swap(
&mut (&mut *self.write_guards_map.get()).get_mut(node).unwrap()[i],
&mut None,
);
} else {
mem::swap(
&mut (&mut *self.read_guards_map.get()).get_mut(node).unwrap()[i],
&mut None,
);
}
}
...
for value in self.graph.node_map.values() {
}
}
...
pub fn eval<'slice, 'feed>(
&mut self,
tensors: &'tensor [T],
feeds: &'slice [Feed<'tensor, 'feed, F>],
...
let evaluated_node = unsafe {
//                self.recv(&mut self.storage, &mut guards)

let OpEvalResult { node, ys } = self.receiver.recv().unwrap();

for (i, input) in node.inputs.iter().enumerate() {
guards.invalidate(input.mut_usage, node, i);
}
...
self.storage.owned_value_storage.push(Some(val));
ValueInfo::new(ValueType::Owned, self.storage.owned_value_storage.len() - 1)
}
Ok(crate::ArrRepr::View(val)) => {
self.storage.borrowed_value_storage.push(val);
ValueInfo::new(ValueType::View, self.storage.borrowed_value_storage.len() - 1)
}
_ => {
ValueInfo::new(ValueType::Empty, 0)
}
};
info_list.push(info);
}
self.storage.value_info_storage.insert(node, info_list);
}
self.graph.node_map.get(&node).unwrap()
};

if is_eval_target(evaluated_node.inner, tensors) {
targets_remaining -= 1;
...
for &suc in &evaluated_node.successors {
let mut suc_info = self.graph.node_map.get_mut(&suc).unwrap();
...
let info = &self.storage.value_info_storage.get(&&in_node.val).unwrap()[in_idx];
match info.ty {
ValueType::Owned => unsafe {
xs.push(OpInput::new(self.storage.owned_value_storage[info.key].as_ref().unwrap().view()));
},
ValueType::View => {
// Clone the view
xs.push(OpInput::new(self.storage.borrowed_value_storage[info.key].clone()));
}
ValueType::Empty => {
panic!(
...
for t in tensors {
let t = t.as_ref();
let arr = if let Some(per) = t.clone_persistent_array() { // rare case
Some(per)
} else if t.is_placeholder { // rare case
let mut found = None;
for feed in feeds {
...
let info = &self.storage.value_info_storage.get(&t).unwrap()[0];
if ValueType::Owned == info.ty {
mem::replace(&mut self.storage.owned_value_storage[info.key], None)
} else if ValueType::View == info.ty {
Some(self.storage.borrowed_value_storage[info.key].to_owned())
} else {
None
}
};
...
ValueInfo::new(ValueType::Owned, storage.owned_value_storage.len() - 1)
}
Ok(crate::ArrRepr::View(val)) => {
storage.borrowed_value_storage.push(val);
ValueInfo::new(ValueType::View, storage.borrowed_value_storage.len() - 1)
}
_ => {
ValueInfo::new(ValueType::Empty, 0)
}
...
pub fn eval<'view, 'slice, 'node, 'feed, T, F>(
tensors: &'node [T],
...
//
////    let (sender, receiver): (Sender<OpEvalResult<'node, 'view, F>>, Receiver<OpEvalResult<'node, 'view, F>>) = crossbeam_channel::bounded(16);
//    let (sender, receiver) = crossbeam_channel::bounded(16);
//
//    {
//        // Views of owned arrays whose lifetime is shorter than owned ones
//        let mut view_storage = Vec::<NdArrayView<'view, F>>::new();
//
//
...
//                let suc_info = graph.node_map.get_mut(&suc).unwrap();
////            suc_info.decrement_pending_count();
//
//                if !suc_info.scheduled() && suc_info.ready() {
//                    suc_info.mark_scheduled();
